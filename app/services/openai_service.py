...
- Yêu cầu soạn tin/văn bản để gửi cho ai đó: CHỈ trả nội dung soạn, sẵn sàng copy-paste; không bọc meta.
...
- KHÔNG dùng các câu meta như: "Đây là tin nhắn cho bạn", "Tôi đã soạn...", "Dưới đây là...",
...
Trả lời bằng tiếng Việt, ngắn gọn, gạch đầu dòng: ý chính, quyết định/hành động (nếu có), chủ đề nổi bật.
...
async def summarize_messages(messages: list[ChatMessage]) -> str:
    request_messages = [ChatMessage(role="system", content=SUMMARIZE_SYSTEM_PROMPT), *messages]
